...
export default function FooterLayout() {
  ...
  const handleSubscribe = async () => {
    ...
    setStatus(null);
    try {
      await instance.post('/newsletter/subscribe', { email });
      setStatus('success');
      setEmail('');
    } catch {
      setStatus('error');
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-16 mb-20 md:mb-32">
          ...
          <div className="md:col-span-3 flex flex-col gap-6 md:gap-8">
            ...
            <ul className="space-y-3 md:space-y-4 font-bold text-white/40 text-sm uppercase tracking-[0.14em]">
              <li><button onClick={() => navigate('/')} className="hover:text-white transition-colors">{t('pages.landing.footer.gallery')}</button></li>
              <li><button onClick={() => navigate('/Agenda')} className="hover:text-white transition-colors">{t('pages.landing.footer.program')}</button></li>
              <li><button onClick={() => navigate('/competition')} className="hover:text-white transition-colors">{t('pages.landing.footer.top50')}</button></li>
              <li><button onClick={() => navigate('/ticket')} className="hover:text-white transition-colors">{t('pages.landing.footer.ticketing')}</button></li>
            </ul>
          </div>
            ...
            <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-8 md:p-10 rounded-[32px] md:rounded-[40px]">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={loading}
                  className="bg-white text-black px-5 md:px-6 py-3 md:py-4 rounded-2xl font-black uppercase text-xs tracking-[0.14em] hover:bg-purple-500 hover:text-white transition-all disabled:opacity-50"
                >
                  {loading ? '...' : t('pages.landing.footer.ok')}
                </button>
              </div>
              {status === 'success' && (
