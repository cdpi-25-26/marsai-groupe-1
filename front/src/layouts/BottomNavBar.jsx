...
export function BottomNavigation() {
  ...
  return (
    <>
      ...
      <div className="fixed bottom-4 md:bottom-6 left-1/2 -translate-x-1/2 z-[100] w-[calc(100%-32px)] max-w-md">
        <motion.div 
          initial={{ y: 100, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ delay: 0.2, type: "spring", damping: 25, stiffness: 200 }}
          className="bg-white/[0.03] backdrop-blur-[40px] border border-white/10 rounded-[40px] p-1.5 md:p-2 shadow-2xl shadow-black/60 relative overflow-hidden"
        >
          {/* Specular Highlight */}
            ...
            {navItems.map((item) => {
              ...
              if (item.isSpecial) {
                return (
                  ...
                  >
                    ...
                    <div className="relative bg-gradient-to-br from-[#A855F7] to-[#51A2FF] rounded-full p-3 md:p-4 shadow-2xl border border-white/20">
                      <Icon className="w-4 h-4 md:w-7 md:h-7 text-white" strokeWidth={3} />
                    </div>
                  </motion.button>
                );
              }
              ...
              return (
                <motion.button
                  ...
                  whileTap={{ scale: 0.85 }}
                  onClick={() => handleNav(item.path)}
                  className="flex flex-col items-center justify-center w-12 h-12 md:w-14 md:h-14 relative cursor-pointer group"
                  title={item.label}
                >
                  <motion.div
                    animate={{
