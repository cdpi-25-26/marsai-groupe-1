...
import { Search, Home, Trophy, Calendar, User, UserPlus, Menu, X } from "lucide-react";
...
export function TopBar() {
  ...
  return (
    <>
      ...
      >
        <div className="bg-white/[0.03] backdrop-blur-[40px] rounded-full h-[56px] md:h-[72px] flex items-center justify-between px-4 md:px-8 gap-2 md:gap-6 shadow-2xl shadow-black/60 pointer-events-auto w-full max-w-[1400px] relative overflow-hidden">
          {/* Specular Highlight */}
              ...
              >
                <item.icon
                  className={`w-5 h-5 transition-all ${
                    isActive(item.path) ? "text-[#51A2FF]" : "text-white/40 group-hover:text-white"
                  }`}
                />
                {isActive(item.path) && (
                  <motion.div
