  ...
  return (
    <div className="min-h-screen text-foreground pb-32 selection:bg-purple-500/30">
      ...
      <div className="relative pt-8 md:pt-16 pb-16 px-6" style={{ overflow: "hidden" }}>
        {/* Ambiance lights — clipped dans le hero, pas de débordement page */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 mb-16">
        {/* Mobile: rank 1 on top, 2+3 side by side */}
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex items-center justify-between mb-8 border-b border-white/10 pb-6">
          ...
          <span className="hidden sm:block text-[10px] font-black uppercase tracking-[0.2em] text-white/30">
            {t("pages.competition.performance")}
          </span>
        </div>
...
function PodiumCard({ film, rank, color, delay, isWinner, mobile, onClick, formatNumber }) {
    ...
    return (
      ...
      >
        <div className="relative overflow-hidden rounded-[20px] bg-white/[0.03] border border-white/10 group-hover:border-white/25 transition-all duration-300 shadow-xl">
          <div className={`aspect-[3/4] relative overflow-hidden ${isWinner ? "" : ""}`}>
            <img
              src={film?.thumbnail}
              alt={film?.title}
            ...
            <div className="absolute bottom-0 left-0 right-0 p-3">
              <h3 className="font-black uppercase tracking-tighter leading-none text-sm truncate mb-0.5">
                {film?.title}
              </h3>
              <p className="text-[#51A2FF] text-[10px] font-black tracking-tight truncate mb-2">
                @{film?.directorUsername}
              </p>
              <div className="flex items-center gap-3">
                <span className="flex items-center gap-1">
                  <Heart className="w-3 h-3 text-pink-500 fill-pink-500/30" />
                  <span className="text-[10px] font-black">{formatNumber(film?.likes)}</span>
                </span>
                <span className="flex items-center gap-1 text-[#51A2FF]">
                  <Eye className="w-3 h-3" />
                  <span className="text-[10px] font-black text-white">{formatNumber(film?.views)}</span>
                </span>
              </div>
            </div>
          </div>
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className={`relative cursor-pointer group ${isWinner ? "w-[340px]" : "w-[260px]"}`}
    >
      {isWinner && (
        <div className="absolute -inset-4 bg-purple-500/10 blur-[60px] rounded-full animate-pulse -z-10" />
      )}
      ...
      <div className="relative overflow-hidden rounded-[28px] bg-white/[0.03] backdrop-blur-2xl border border-white/10 transition-all duration-500 group-hover:border-white/30 group-hover:-translate-y-2 group-hover:bg-white/[0.05] shadow-2xl">
        ...
        <div className={`p-5 ${isWinner ? "sm:p-7" : "sm:p-5"}`}>
          <h3 className={`font-black uppercase tracking-tighter leading-none mb-1 truncate ${isWinner ? "text-2xl" : "text-lg"}`}>
            {film?.title}
          </h3>
          <p className="text-[#51A2FF] text-xs font-black tracking-tight mb-4">
            @{film?.directorUsername}
          </p>
          <div className="flex items-center gap-5 border-t border-white/5 pt-4">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs font-black">{formatNumber(film?.likes)}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <Eye className="w-3.5 h-3.5 text-[#51A2FF]" />
              <span className="text-xs font-black">{formatNumber(film?.views)}</span>
            </div>
          </div>
        </div>
      </div>
...
function RankRow({ film, rank, onClick, formatNumber, t }) {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      onClick={onClick}
      className="group flex items-center gap-3 sm:gap-5 p-3 sm:p-4 rounded-[16px] sm:rounded-[20px] bg-white/[0.02] border border-white/5 hover:bg-white/[0.05] hover:border-[#51A2FF]/30 transition-all duration-300 cursor-pointer"
    >
      {/* Rank */}
      <div className={`w-7 sm:w-9 text-center font-black text-sm sm:text-base shrink-0 ${isTop10 ? "text-[#51A2FF]" : "text-white/20 group-hover:text-[#51A2FF]"}`}>
        {rank}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <h4 className="font-black text-xs sm:text-base truncate group-hover:text-[#51A2FF] transition-colors uppercase tracking-tight leading-none mb-0.5">
          {film.title}
        </h4>
