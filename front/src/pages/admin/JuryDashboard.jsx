...
export default function JuryDashboard({ onNavigate }) {
  ...
  const finalistsFilms = filmsData.filter(f => f.rank && f.rank <= 50).map(film => ({
    ...film,
    hasVoted: Math.random() > 0.5, // Simulé
    myRating: Math.random() > 0.5 ? Math.floor(Math.random() * 4) : undefined
  }));

  // Statistiques
  const evaluatedCount = finalistsFilms.filter(f => f.hasVoted).length;
  const totalCount = finalistsFilms.length;
  const progressPercent = Math.round((evaluatedCount / totalCount) * 100);

  // Filtres et tri
  const filteredFilms = useMemo(() => {
    let films = [...finalistsFilms];
    ...
    if (filterStatus === 'evaluated') {
      films = films.filter(f => f.hasVoted);
    ...
    if (sortBy === 'title') {
      ...
      films.sort((a, b) => a.duration.localeCompare(b.duration));
    }
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white overflow-y-auto pb-32">
      {/* Background Ambience - Couleurs cinématographiques */}
      ...
      <div className="sticky top-0 z-40 bg-[#050505]/60 backdrop-blur-3xl border-b border-[#51a2ff]/10">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 md:py-8">
            ...
            <div className="flex items-center gap-4 mb-3">
              ...
              <div>
                ...
                <p className="text-[#51a2ff]/60 text-xs md:text-sm font-medium uppercase tracking-[0.2em]">
                  Évaluation des 50 finalistes
                </p>
              </div>
            </div>
          </motion.div>

          {/* Progress Bar - Glass UI avec accent cyan */}
          <motion.div
          ...
          >
            ...
            {/* Progress bar avec gradient cyan subtil */}
          ...
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <div className="relative md:col-span-1">
              ...
              <input
                ...
                className="w-full bg-white/[0.03] backdrop-blur-xl border border-white/[0.08] rounded-[24px] pl-12 pr-4 py-3.5 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-[#ad46ff]/30 focus:bg-white/[0.05] focus:shadow-[0_0_20px_rgba(173,70,255,0.1)] transition-all"
              />
            </div>

            {/* Status Filter avec accents couleur */}
                ...
                <button
                  ...
                  className={`flex-1 px-4 py-3 rounded-[24px] text-xs font-black uppercase tracking-widest transition-all border backdrop-blur-xl ${
                    filterStatus === filter.id
                      ...
                      : 'bg-white/[0.02] text-white/40 hover:border-white/15 hover:bg-white/[0.04]'
                  }`}
                >
                  {filter.label}
            ...
            <select
              ...
              className="bg-white/[0.03] backdrop-blur-xl border border-white/[0.08] rounded-[24px] px-4 py-3.5 text-sm text-white focus:outline-none focus:border-[#51a2ff]/30 focus:bg-white/[0.05] appearance-none cursor-pointer transition-all"
            >
            ...
            >
              <div className="relative bg-white/[0.02] border border-white/5 rounded-3xl overflow-hidden hover:bg-white/[0.05] hover:border-white/20 transition-all">
                ...
                <div className="aspect-[4/5] relative overflow-hidden">
                  ...
                  {/* My Rating Badge */}
                  {film.hasVoted && film.myRating !== undefined && (
                  ...
                  {/* Rank Badge */}
                  {film.rank && (
                    <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-md rounded-xl px-3 py-1.5 border border-white/10">
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center justify-between text-xs text-white/40 mb-3">
                    <span className="font-medium">{film.category}</span>
                  ...
                  <div className="flex flex-wrap gap-1">
                    ...
                    {film.aiTools.length > 2 && (
                      <span className="bg-white/5 px-2 py-1 rounded-lg text-[9px] font-bold text-white/60">
                        +{film.aiTools.length - 2}
                      </span>
                    )}
                  </div>
